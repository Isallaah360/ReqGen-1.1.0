...
export default function ExecutiveAnalyticsPage() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.analyticsGrid}>
      <article className={styles.card}><h2 className={styles.cardTitle}>Expenditure Trend · FY {year}</h2><div className={styles.bars}>{monthly.map(m=><div className={styles.barRow} key={m.month} tabIndex={0} title={`${m.month}: ${money(m.value)}`}><span className={styles.barLabel}>{m.month}</span><div className={styles.barTrack}><i className={styles.barFill} style={{width:`${m.value/maxMonth*100}%`}}/></div><strong className={styles.barValue}>{money(m.value)}</strong></div>)}</div><div className={styles.notice} style={{marginTop:12,marginBottom:0}}>Trend uses actual classified outflows from <strong>finance_transactions</strong>. No synthetic months are drawn.</div></article>
      <article className={styles.card}><h2 className={styles.cardTitle}>Expenditure by Department</h2><div className={styles.bars}>{deptSpend.map(d=><div className={styles.barRow} key={d.name} tabIndex={0} title={`${d.name}: ${money(d.spend)}`}><span className={styles.barLabel}>{d.name}</span><div className={styles.barTrack}><i className={styles.barFill} style={{width:`${d.spend/maxDept*100}%`}}/></div><strong className={styles.barValue}>{money(d.spend)}</strong></div>)}</div>{!deptSpend.length?<div className={styles.empty}>No departments are available.</div>:null}</article>
