...
function requestedOtpChannel(): OtpChannel {
  return normalizeOtpChannel(
    process.env.NEXT_PUBLIC_REQGEN_REQUEST_OTP_CHANNEL ||
    "sms_email"
  );
}
...
export async function POST(req: NextRequest) {
  ...
  const debug = {
    rawConfiguredChannel:
      process.env
        .NEXT_PUBLIC_REQGEN_REQUEST_OTP_CHANNEL ||
      null,
  ...
  const smsMessage =
    buildOtpSmsMessage(otp);

  const emailSubject =
    "IET ReqGen OTP Verification Code";
  ...
  if (!delivered) {
    ...
    return jsonError(
      smsError ||
      emailError ||
      "OTP could not be delivered through the selected channel.",
      500,
      {
        debug: {
          ...debug,
  ...
  return NextResponse.json({
    ok: true,
    message:
      `OTP sent successfully by ${deliveryLabel}.`,
    channel:
      smsSent && emailSent
        ? "sms_email"
        : smsSent
