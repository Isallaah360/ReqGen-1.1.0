...
export default function DepartmentsPage() {
  ...
  const canManage = roleKey(myRole) === "admin";

  const [rows, setRows] = useState<Dept[]>([]);
  const [routingUsers, setRoutingUsers] = useState<ProfileMini[]>([]);
  ...
  const load = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        const { data: routingRows, error: routingErr } = await supabase
          .from("profiles")
        ...
        setRoutingUsers((routingRows || []) as ProfileMini[]);
        ...
        setMessage(
          `Unable to load departments: ${errorMessage(
            error
          )}`
        );
      } finally {
  ...
  function openEdit(department: Dept) {
    ...
    setDirectorUserId(department.director_user_id || "");
    setFormOpen(true);
  }
      ...
      if (editId) {
        const { error } = await supabase
          ...
          .update({
            ...
            director_user_id: directorUserId || null,
          })
          .eq("id", editId);
  ...
  function exportDepartments() {
    ...
    try {
      exportTableToExcel({
        ...
        title:
          "Department Management Register",

        subtitle:
          `${REQGEN_PRODUCT_LABEL} — Islamic Education Trust`,
        ...
        >
          ...
          >
            ...
            <div className={styles.detailGrid}>
              ...
              <Detail
                label="HOD"
                value={routingUserLabel(selected.hod_user_id)}
              />

              <Detail
                label="Director"
                value={routingUserLabel(selected.director_user_id)}
              />
        ...
        >
          ...
          >
            ...
            <div className={styles.formGrid}>
              ...
              <label>
                <span>HOD</span>
                <select value={hodUserId} onChange={(event) => setHodUserId(event.target.value)} disabled={saving || !canManage}>
              ...
              <label>
                <span>Director</span>
                <select value={directorUserId} onChange={(event) => setDirectorUserId(event.target.value)} disabled={saving || !canManage}>
                  <option value="">None</option>
                  {routingUsers.map((user) => (
                    <option key={user.id} value={user.id}>
                      {user.full_name || user.email || user.id}{user.role ? ` (${user.role})` : ""}
