...
function activeRoleFromRpc(value: unknown): string {
  if (typeof value === "string") return normalizeRole(value);

  if (Array.isArray(value)) {
    for (const item of value) {
      const parsed = activeRoleFromRpc(item);
  ...
  if (value && typeof value === "object") {
    ...
    return normalizeRole(
      record.active_role_key ??
        record.role_key ??
        record.active_role ??
        record.role ??
        record.get_my_active_role
    );
  }
...
export default function AdminAuditPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [profileResult, activeRoleResult] = await Promise.all([
        supabase.from("profiles").select("role").eq("id", auth.user.id).maybeSingle(),
        supabase.rpc("get_my_active_role"),
      ]);

      const profileRole = normalizeRole(profileResult.data?.role);
      const selectedRole = activeRoleFromRpc(activeRoleResult.data);
      const role = selectedRole || profileRole;

      if (activeRoleResult.error) {
        console.warn("Unable to read the selected active role; using the profile role fallback.", activeRoleResult.error);
      }

      if (!["admin", "auditor"].includes(role)) {
        router.replace(`/unauthorized?from=${encodeURIComponent("/admin/audit")}&role=${encodeURIComponent(role || "staff")}`);
        return;
      }
