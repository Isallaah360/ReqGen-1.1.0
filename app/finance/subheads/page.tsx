...
function getParentLabel(
  subhead: Subhead,
  byCode: Map<string, Subhead>
) {
  const code = parentCode(subhead.code);
...
export default function FinanceSubheadsPage() {
      ...
      try {
        ...
        const [
          ...
          supabase
            .from("iet_accounts")
            .select(
              "id,code,name,bank_name,is_active,total_fund,unallocated_balance,available_balance"
            )
            .order("name", {
              ascending: true,
            }),
        ...
        if (deptRes.error) {
          throw new Error(deptRes.error.message);
        }
        ...
        const freshSubheads =
          ((subRes.data || []) as Subhead[]).map(
            (item) => ({
              ...item,
              approved_allocation: Number(
                item.approved_allocation || 0
              ),
              reserved_amount: Number(
        ...
        const requestCountBySubhead: Record<
          string,
          number
        > = {};
        ...
        setDepartments(
          (deptRes.data || []) as Dept[]
        );

        setBanks(
          (bankRes.data || []) as BankAccount[]
        );
        ...
        setMessage(
          `Unable to load finance subheads: ${errorMessage(
            error
          )}`
        );
      } finally {
  ...
  async function saveSubhead() {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          `Allocation cannot be below committed amount (${money(
            committed
          )}).`
        );
      }
      ...
      if (
        ...
        throw new Error(
          `Allocation exceeds available bank capacity (${money(
            availableCapacity
          )}).`
        );
      }
      ...
      setMessage(
        editId
          ? "Subhead updated successfully."
          : "New subhead created successfully."
      );
  ...
  async function toggleActive(
    ...
    try {
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "Unable to update subhead status."
      );
    } finally {
      setSaving(false);
    }
  ...
  async function removeUnused(
    ...
    if (linked) {
      setMessage(
        "This subhead has financial or request activity. Deactivate it instead of deleting it."
      );
      return;
    }
  ...
  async function exportCsv() {
    ...
    try {
      exportTableToExcel<Subhead>({
        ...
        subtitle:
          `${filtered.length} visible subheads | Allocation ${money(
            allocationTotal
          )}`,
    ...
    <div className={styles.page}>
      <header className={styles.pageHeader}>
        <div>
          ...
          <p>
            Create, manage and organize all finance subheads and budgeting categories.
          </p>
        </div>
      ...
      <section className={styles.kpiGrid}>
        <Kpi
          ...
          tone="blue"
          label="Total Subheads"
          value={String(total)}
          note="All Finance Subheads"
        />

        <Kpi
          icon={<ShieldCheck size={24} />}
          tone="green"
          label="Active Subheads"
        ...
        <Kpi
          ...
          tone="purple"
          label="Under Subheads"
          value={String(childCount)}
          note="Subheads with parent items"
        />
      </section>

      <section className={styles.filterCard}>
        <FilterField
          label="Search Subhead"
          grow
        >
      ...
      <section className={styles.contentGrid}>
        <div className={styles.tableCard}>
          <div className={styles.cardHeader}>
            <div>
              <h2>
                Finance Subheads ({filtered.length})
              </h2>
            </div>
                ...
                {paged.length === 0 ? (
                  <tr>
                    <td colSpan={9}>
                      <div className={styles.empty}>
                        No finance subheads match the selected filters.
                      </div>
                    </td>
                  </tr>
                ) : (
                  paged.map((item, index) => (
                    <tr key={item.id}>
                      ...
                      <td>
                        <span
                          className={`${styles.statusPill} ${item.is_active
                              ? styles.active
                              : styles.inactive
                            }`}
                        >
        ...
        <aside className={styles.sideColumn}>
          ...
          <section className={styles.sideCard}>
            ...
            <QuickAction
              icon={<Plus size={16} />}
              tone="blue"
              title="Create New Subhead"
              note="Add a new finance subhead"
              onClick={openCreate}
            />
            ...
            <QuickAction
              ...
              tone="green"
              title="Manage Parent Heads"
              note="Review top-level budget heads"
              onClick={() => {
                setLevel("1");
                setParent("all");
              }}
            ...
            <QuickAction
              ...
              tone="purple"
              title="View Subhead Hierarchy"
              note="Browse subheads by parent"
              onClick={() => {
                setLevel("all");
                setParent(
                  parentOptions[0]?.code ||
                  "all"
                );
              }}
            />
            ...
            <QuickAction
              ...
              note="Allocate budgets to subheads"
              onClick={() =>
                router.push(
            ...
            <QuickAction
              ...
              tone="green"
              title="Export Subhead List"
              note="Download finance subheads report"
              onClick={() =>
                void exportCsv()
              }
            />
          ...
          <section className={styles.noteCard}>
            ...
            <p>
              Subheads are used for budgeting, reporting and financial control.
            </p>

            <p>
              Changes may affect existing budgets and transactions.
            </p>
          </section>
        </aside>
      </section>
            ...
            <div className={styles.formGrid}>
              ...
              <label className={styles.fullField}>
                ...
                <textarea
                  ...
                  placeholder="Purpose or allocation note"
                  rows={3}
                />
              </label>
            </div>
...
function FilterField({
  ...
  return (
    <label
      className={`${styles.filterField} ${grow
          ? styles.grow
          : ""
        }`}
    >
