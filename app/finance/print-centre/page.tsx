      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_.72fr]">
        <FinanceCard>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {templates.map((item) => {
              ...
              return <button key={item.id} onClick={() => setSelected(item.id)} className={`group rounded-2xl border p-4 text-left transition ${active ? "border-cyan-500 bg-cyan-50 ring-4 ring-cyan-100" : "border-slate-300 bg-white hover:-translate-y-0.5 hover:border-cyan-300 hover:shadow-md"} duration-200 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-200 disabled:cursor-not-allowed disabled:opacity-60 text-slate-900`}><div className="flex items-start gap-3"><span className={`flex h-11 w-11 items-center justify-center rounded-2xl text-xl ${active ? "bg-cyan-700 text-white" : "bg-slate-100"}`}>{item.icon}</span><span><span className="block font-black text-slate-950">{item.title}</span><span className="mt-1 block text-xs font-semibold leading-5 text-slate-500">{item.text}</span></span></div></button>;
            })}
          </div>
        </FinanceCard>
        ...
        <FinanceCard>
          ...
          <div className="mt-5 grid gap-3">
            <Link href={`/finance/reports?view=output&template=${selected}`} className="inline-flex items-center justify-center gap-2 rounded-xl bg-cyan-700 px-4 py-3 text-sm font-black text-white shadow-sm transition hover:bg-cyan-800 duration-200 hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-200 disabled:cursor-not-allowed disabled:opacity-60">👁️ Open Print Workspace</Link>
