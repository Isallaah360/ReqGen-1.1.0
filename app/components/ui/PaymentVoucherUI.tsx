...
export function PVActionButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`reqgen-btn reqgen-btn-blue inline-flex min-h-12 min-w-[150px] items-center justify-center rounded-xl border px-5 py-3 text-sm font-black tracking-[0.01em] shadow-md backdrop-blur-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-lg focus:outline-none focus:ring-4 focus:ring-blue-200 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:translate-y-0 ${tones[tone]}`}
    >
      {children}
    </button>
  );
