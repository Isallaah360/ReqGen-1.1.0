...
export function ActionButton({ href, onClick, icon = "arrow", children, variant = "light", disabled = false }: { href?: string; onClick?: () => void; icon?: string; children: ReactNode; variant?: "light" | "primary" | "ghost"; disabled?: boolean }) {
  ...
  if (href) return <Link href={href} className={`reqgen-btn reqgen-btn-blue inline-flex h-11 items-center justify-center gap-2 rounded-xl px-4 text-sm font-bold transition duration-200 hover:-translate-y-0.5 ${cls}`}>{content}</Link>;
  return <button type="button" onClick={onClick} disabled={disabled} className={`reqgen-btn reqgen-btn-blue inline-flex h-11 items-center justify-center gap-2 rounded-xl px-4 text-sm font-bold transition duration-200 hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60 ${cls}`}>{content}</button>;
}
