...
export function ReportButton({ children, onClick, icon, variant = "blue", disabled = false }: { children: ReactNode; onClick?: () => void; icon: string; variant?: "light" | "blue" | "violet" | "cyan"; disabled?: boolean }) {
  ...
  return <button type="button" onClick={onClick} disabled={disabled} className={`reqgen-btn reqgen-btn-blue inline-flex h-11 min-w-[132px] items-center justify-center gap-2 rounded-xl px-4 text-sm font-extrabold shadow-lg transition duration-200 hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60 ${variants[variant]}`}><ReportIcon name={icon} className="h-4 w-4"/>{children}</button>;
}
