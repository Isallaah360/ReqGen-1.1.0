    ...
    <section className="mx-auto mb-6 w-full max-w-6xl rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
        {visibleItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className={`group flex min-h-14 items-center gap-3 rounded-xl bg-gradient-to-r ${item.tone} px-3 py-2.5 font-black text-white shadow-md transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg focus:outline-none focus:ring-4 focus:ring-blue-200 ${active ? "ring-4 ring-sky-200" : ""}`}>
              <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/15 ring-1 ring-white/20">{item.icon}</span>
