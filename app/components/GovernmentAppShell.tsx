...
export default function GovernmentAppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`gov-shell gov-module-${moduleKey(pathname)}`}>
      <button
      ...
      <aside className={`gov-sidebar ${mobileOpen ? "is-open" : ""}`} aria-label="Primary navigation">
        <div className="gov-brand">
          <Image src="/iet-logo.png" alt="Islamic Education Trust" width={60} height={60} className="gov-brand-logo" priority />
          <div>
        ...
        <header className="gov-topbar">
          ...
          <div className="gov-search-wrap">
            <button className="gov-search-trigger" onClick={() => setSearchOpen((value) => !value)}>
              <Search size={18} />
              <span>Search requests, reports, documents...</span>
            </button>
            {searchOpen && (
              <div className="gov-search-popover">
                <div className="gov-search-input-wrap"><Search size={18} /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search authorised ReqGen pages and functions..." /></div>
          ...
          <div className="gov-topbar-actions">
            <Link href="/staff/notifications" className="gov-icon-button gov-badge-button" aria-label="Notifications">
              <Bell size={18} />
              <span>8</span>
            </Link>
            <Link href="/staff/notifications" className="gov-icon-button gov-badge-button gov-message-button" aria-label="Messages">
