...
export default function RegistryCentreWorkspace() {
  ...
  const [loading,setLoading]=useState(true); const [message,setMessage]=useState<string|null>(null); const [showForm,setShowForm]=useState(false); const [chartDetail,setChartDetail]=useState<string|null>(null);
  ...
  return <main className={styles.page} data-rg-standard="phase7"><div className={styles.shell}>
    ...
    {view==="overview"&&<>
      ...
      <section className={styles.grid3}><div className={styles.card}><h2 className={styles.cardTitle}>Incoming vs Outgoing Trend</h2><p className={styles.cardNote}>Six-week live correspondence trend.</p>{rows.length?<button type="button" className={styles.chartButton} onClick={()=>setChartDetail(trend.map(t=>`${t.label}: ${t.incoming} incoming / ${t.outgoing} outgoing`).join(" · "))}><svg className={styles.chart} viewBox="0 0 200 100" preserveAspectRatio="none" role="img" aria-label="Incoming versus outgoing correspondence trend"><title>{`Incoming ${trend.reduce((a,b)=>a+b.incoming,0)}; Outgoing ${trend.reduce((a,b)=>a+b.outgoing,0)}`}</title><g stroke="#e6edf5" strokeWidth=".6">{[20,40,60,80].map(y=><line key={y} x1="8" y1={y} x2="194" y2={y}/>)}</g><polyline fill="none" stroke="#1263f3" strokeWidth="2" points={points("incoming")}/><polyline fill="none" stroke="#11a676" strokeWidth="2" points={points("outgoing")}/></svg></button>:<div className={styles.empty}>No Registry trend data yet.</div>}<div className={styles.legend}><div className={styles.legendRow}><i className={styles.dot} style={{background:"#1263f3"}}/><span>Incoming</span><strong>{trend.reduce((a,b)=>a+b.incoming,0)}</strong></div><div className={styles.legendRow}><i className={styles.dot} style={{background:"#11a676"}}/><span>Outgoing</span><strong>{trend.reduce((a,b)=>a+b.outgoing,0)}</strong></div></div></div>
      <div className={styles.card}><h2 className={styles.cardTitle}>Correspondence by Department</h2><p className={styles.cardNote}>All departments represented from live Registry records.</p>{depStats.length?<div className={styles.donutWrap}><button type="button" className={styles.donut} aria-label="Department correspondence chart. Select for exact values." title={depStats.map(x=>`${x.name}: ${x.value}`).join(" · ")} onClick={()=>setChartDetail(depStats.map(x=>`${x.name}: ${x.value}`).join(" · "))} style={{background:donut(depStats)}}/><div className={styles.legend}>{depStats.map((x,i)=><div key={x.name} className={styles.legendRow}><i className={styles.dot} style={{background:COLORS[i%COLORS.length]}}/><span>{x.name}</span><strong>{x.value}</strong></div>)}</div></div>:<div className={styles.empty}>No department correspondence yet.</div>}</div>
      <div className={styles.card}><h2 className={styles.cardTitle}>Correspondence by Status</h2><p className={styles.cardNote}>Current movement status across all records.</p>{statusStats.length?<div className={styles.donutWrap}><button type="button" className={styles.donut} aria-label="Correspondence status chart. Select for exact values." title={statusStats.map(x=>`${x.name}: ${x.value}`).join(" · ")} onClick={()=>setChartDetail(statusStats.map(x=>`${x.name}: ${x.value}`).join(" · "))} style={{background:donut(statusStats)}}/><div className={styles.legend}>{statusStats.map((x,i)=><div key={x.name} className={styles.legendRow}><i className={styles.dot} style={{background:COLORS[i%COLORS.length]}}/><span>{x.name}</span><strong>{x.value}</strong></div>)}</div></div>:<div className={styles.empty}>No status data yet.</div>}</div></section>{chartDetail&&<div className={styles.chartDetail} role="status" aria-live="polite">{chartDetail}</div>}
    </>}
