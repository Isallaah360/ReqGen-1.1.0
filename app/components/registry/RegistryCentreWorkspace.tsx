...
function statusTone(value: string) {
  ...
  if (/delivered|complete|approved|collected|closed|paid/.test(x)) return { bg: "#e9fbf3", color: "#087247" };
  if (/await|pending|draft|progress/.test(x)) return { bg: "#fff6dd", color: "#9a6200" };
  if (/overdue|reject|cancel|missing/.test(x)) return { bg: "#fff0f0", color: "#b4232f" };
  if (/dispatch/.test(x)) return { bg: "#f3efff", color: "#6842c2" };
  return { bg: "#eef5ff", color: "#1459b5" };
}
function priorityTone(value: string) {
  return /urgent|high/i.test(value)
    ? { bg: "#fff0f0", color: "#c02b38" }
    : /low/i.test(value)
      ? { bg: "#eef5ff", color: "#2762b3" }
      : { bg: "#fff6dd", color: "#936100" };
}
...
export default function RegistryCentreWorkspace() {
  ...
  return <main className={styles.page} data-rg-standard="phase7"><div className={styles.shell}>
    ...
    <nav className={styles.tabs} data-rg-tabs="true" aria-label="Registry Centre workspaces">
      {VIEWS.map((v) => <button key={v.key} aria-selected={view === v.key} className={`${styles.tab} ${view === v.key ? styles.tabActive : ""}`} onClick={() => { setPage(1); router.replace(`/registry?view=${v.key}`); }}>{v.label}</button>)}
    </nav>
