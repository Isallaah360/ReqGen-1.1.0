...
function CommandPalette({ open, onClose, items }: { open: boolean; onClose: () => void; items: ModuleConfig[] }) {
  ...
  if (!open) return null;
  const filtered = items.filter(item => `${item.label} ${item.title}`.toLowerCase().includes(query.toLowerCase()));
...
export default function LegacyERP({ module }: { module?: string } = {}) {
  ...
  const routeModule = pathname.split("/").filter(Boolean)[1] || "dashboard";
  const active = useMemo(() => modules.find(item => item.key === (module || routeModule)) || modules[0], [module, routeModule]);
  ...
  const [accessibleModules, setAccessibleModules] = useState<ModuleConfig[]>([]);
  const [navigating, setNavigating] = useState(false);
  const Icon = active.icon;

  useEffect(() => {
    document.body.classList.add("reqgen-erp-canvas");
    return () => document.body.classList.remove("reqgen-erp-canvas");
  }, []);
  ...
  useEffect(() => {
    ...
    async function loadAccess() {
      ...
      if (!active || !context?.activeRoleKey) return;
      const roleSet = new Set([context.activeRoleKey]);
      setAccessibleModules(modules.filter((item) => canAccessPath(legacyWorkspaceRoutes[item.key], roleSet)));
    }
    void loadAccess();
    const refresh = () => void loadAccess();
    window.addEventListener("reqgen-active-role-changed", refresh);
    return () => { active = false; window.removeEventListener("reqgen-active-role-changed", refresh); };
  }, []);
  ...
  return <div className={`erp-shell ${collapsed ? "erp-collapsed" : ""} ${density === "compact" ? "erp-density-compact" : ""}`}><a className="erp-skip-link" href="#erp-main-content">Skip to main content</a>
    <aside className={`erp-sidebar ${mobile ? "erp-mobile-open" : ""}`}><div className="erp-brand"><Image src="/iet-logo.png" alt="IET" width={40} height={40} /><div><strong>REQGEN</strong><span>ERP 2.0</span></div><button className="erp-mobile-close" onClick={() => setMobile(false)} aria-label="Close navigation"><X size={20} /></button></div><div className="erp-sidebar-context"><small>ENTERPRISE WORKSPACE</small><strong>Barderian Enterprises</strong></div><nav>{accessibleModules.filter(item => !["profile", "notifications"].includes(item.key)).map(item => { const NavIcon = item.icon; return <Link href={`/erp-2/${item.key}`} onClick={beginNavigation} className={pathname.endsWith(item.key) ? "active" : ""} key={item.key}><NavIcon size={19} /><span>{item.label}</span>{item.key === "approvals" && <b>45</b>}</Link>; })}</nav><button className="erp-sidebar-command" onClick={() => setCommand(true)}><Search size={16} /><span>Quick search</span><kbd>Ctrl K</kbd></button><div className="erp-sidebar-foot"><span>PRODUCTION</span><small>ReqGen ERP v2.0 · Release 76</small></div></aside>
    {mobile && <button className="erp-mobile-scrim" aria-label="Close navigation" onClick={() => setMobile(false)} />}
    <div className="erp-workspace"><header className="erp-topbar"><div className="erp-topbar-left"><button className="erp-menu-button" onClick={() => { if (window.innerWidth < 900) setMobile(true); else setCollapsed(!collapsed); }} aria-label="Toggle navigation"><Menu size={21} /></button><HeaderGreeting section={active.title.replaceAll("&", "and")} userName={currentUser.fullName} /></div><div className="erp-topbar-centre"><button className="erp-global-search" onClick={() => setCommand(true)}><Search size={17} /><span>Search ReqGen ERP...</span><kbd>Ctrl K</kbd></button><div className="erp-clock"><Clock /></div></div><div className="erp-topbar-actions"><div className="erp-role-switcher"><ActiveRoleSwitcher compact allowInERP /></div><Link href="/erp-2/notifications" onClick={beginNavigation} className="erp-icon-button erp-bell" aria-label="Notifications"><Bell size={19} /><i>5</i></Link><div className="erp-profile"><button onClick={() => setProfile(!profile)} aria-expanded={profile}><span className="erp-avatar">{currentUser.initials}</span><span><strong>{currentUser.fullName}</strong><small>{currentUser.role}</small></span><ChevronDown size={15} /></button>{profile && <div className="erp-profile-menu"><div className="erp-profile-menu-head"><strong>{currentUser.fullName}</strong><small>{currentUser.email || currentUser.role}</small></div><Link href="/erp-2/profile" onClick={beginNavigation}><UserRound size={16} />My Profile</Link><Link href="/erp-2/settings" onClick={beginNavigation}><Settings size={16} />Settings</Link><button onClick={() => setDensity(value => value === "comfortable" ? "compact" : "comfortable")}><SlidersHorizontal size={16} />{density === "comfortable" ? "Compact View" : "Comfortable View"}</button><Link href="/login"><LogOut size={16} />Logout</Link></div>}</div></div></header>
      <main className="erp-main erp-main-unified" id="erp-main-content"><div className="erp-watermark"><Image src="/be-logo.png" alt="" fill sizes="60vw" /></div><section className="erp-page-title"><div><span className="erp-eyebrow">REQGEN ERP 2.0 · PRODUCTION</span><h1>{active.title}</h1><p>{active.subtitle}</p></div><div className="erp-page-title-icon"><Icon size={24} /></div></section><div className="erp-breadcrumb"><Link href="/erp-2/dashboard" onClick={beginNavigation}>Home</Link><span>/</span><strong>{active.label}</strong></div><section className={`erp-content-stage ${navigating ? "is-loading" : ""}`}><ModuleContent kind={active.key} />{navigating && <div className="erp-route-progress" aria-live="polite"><span /><small>Loading {active.label}…</small></div>}</section></main>
      <footer className="erp-footer"><span>© 2026 Barderian Enterprises. All rights reserved.</span><span>ReqGen ERP 2.0 · Production · Release 76</span></footer></div>
