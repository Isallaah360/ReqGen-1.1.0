...
export default function LegacyERP({ module }: { module: string }) {
  ...
  return <div className={`erp-shell ${collapsed ? "erp-collapsed" : ""}`}><a className="erp-skip-link" href="#erp-main-content">Skip to main content</a>
    ...
    <div className="erp-workspace"><header className="erp-topbar"><div className="erp-topbar-left"><button className="erp-menu-button" onClick={() => { if (window.innerWidth < 900) setMobile(true); else setCollapsed(!collapsed); }} aria-label="Toggle navigation"><Menu size={21} /></button><HeaderGreeting section={active.title.replaceAll("&", "and")} /></div><div className="erp-clock"><Clock /></div><div className="erp-topbar-actions"><Link href="/erp-2/notifications" className="erp-icon-button erp-bell" aria-label="Notifications"><Bell size={19} /><i>5</i></Link><div className="erp-profile"><button onClick={() => setProfile(!profile)} aria-expanded={profile}><span className="erp-avatar">I</span><span><strong>Isallaah360</strong><small>Super Administrator</small></span><ChevronDown size={15} /></button>{profile && <div className="erp-profile-menu"><Link href="/erp-2/profile"><UserRound size={16} />My Profile</Link><Link href="/erp-2/settings"><Settings size={16} />Settings</Link><Link href="/login"><LogOut size={16} />Logout</Link></div>}</div></div></header>
      <main className="erp-main" id="erp-main-content"><div className="erp-watermark"><Image src="/be-logo.png" alt="" fill sizes="70vw" /></div><div className="erp-page-head"><div><span className="erp-eyebrow"><Icon size={15} />{active.label}</span><h1>{active.title}</h1><p>{active.subtitle}</p></div><div className="erp-page-actions"><button className="erp-button erp-button-secondary"><Download size={16} />Export</button>{active.action && <button className="erp-button erp-button-gold"><Plus size={16} />{active.action}</button>}</div></div>{!["profile", "settings", "notifications", "reports"].includes(active.key) && <div className="erp-toolbar"><div className="erp-search"><Search size={17} /><input placeholder={`Search ${active.label.toLowerCase()}...`} /></div><select aria-label="Category"><option>All Categories</option></select><select aria-label="Status"><option>All Status</option></select><select aria-label="Period"><option>This Month</option></select><button className="erp-button erp-button-secondary"><SlidersHorizontal size={16} />Filter</button></div>}<ModuleContent kind={active.key} /></main>
      <footer className="erp-footer"><span>© 2026 Barderian Enterprises. All rights reserved.</span><span>ReqGen ERP 2.0 · Production</span></footer></div>
  </div>;
}
