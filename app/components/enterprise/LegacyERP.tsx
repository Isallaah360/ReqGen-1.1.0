...
function EnterpriseModal({ active, onClose, onComplete }: { active: ModuleConfig; onClose: () => void; onComplete: (message: string) => void }) {
  const noun = active.key === "staff" ? "Staff Record" : active.key === "vouchers" ? "Payment Voucher" : "Request";
  return <div className="erp-modal-layer" role="presentation" onMouseDown={onClose}>
    <section className="erp-modal" role="dialog" aria-modal="true" aria-labelledby="erp-modal-title" onMouseDown={event => event.stopPropagation()}>
      <div className="erp-modal-head"><div><span className="erp-eyebrow">QUICK CREATE</span><h2 id="erp-modal-title">NEW {noun.toUpperCase()}</h2><p>Enter the essential information. Additional workflow fields can be completed after saving.</p></div><button className="erp-icon-button" onClick={onClose} aria-label="Close dialog"><X size={18} /></button></div>
      <div className="erp-form-grid">
        <label>Title / Subject<input autoFocus placeholder={`Enter ${noun.toLowerCase()} title`} /></label>
        <label>Department<select defaultValue="Executive Management"><option>Executive Management</option><option>Finance</option><option>Human Resources</option><option>Administration</option></select></label>
        <label>Priority<select defaultValue="Normal"><option>Low</option><option>Normal</option><option>High</option><option>Critical</option></select></label>
        <label>Required Date<input type="date" /></label>
        <label className="erp-field-wide">Description<textarea rows={4} placeholder="Provide a clear business description" /></label>
      </div>
      <div className="erp-modal-actions"><button className="erp-button erp-button-secondary" onClick={onClose}>Cancel</button><button className="erp-button erp-button-gold" onClick={() => onComplete(`${noun} saved successfully`)}><CheckCircle2 size={16} />Save {noun}</button></div>
    </section>
  </div>;
}

function CommandPalette({ onClose }: { onClose: () => void }) {
  return <div className="erp-modal-layer erp-command-layer" role="presentation" onMouseDown={onClose}><section className="erp-command" role="dialog" aria-modal="true" aria-label="ReqGen quick navigation" onMouseDown={event => event.stopPropagation()}><div className="erp-command-search"><Search size={19} /><input autoFocus placeholder="Search modules, records and actions..." /><kbd>ESC</kbd></div><div className="erp-command-results"><span>QUICK NAVIGATION</span>{modules.map(m => { const CommandIcon=m.icon; return <Link href={`/erp-2/${m.key}`} key={m.key} onClick={onClose}><CommandIcon size={17} /><div><strong>{m.label}</strong><small>{m.subtitle}</small></div><ArrowUpRight size={15} /></Link>; })}</div></section></div>;
}

function ModuleContent({ kind }: { kind: ModuleKey }) { if (kind === "dashboard") return <Dashboard />; if (["requests", "approvals", "vouchers", "staff"].includes(kind)) return <Requests kind={kind as "requests" | "approvals" | "vouchers" | "staff"} />; if (kind === "finance") return <Finance />; if (kind === "reports") return <Reports />; if (kind === "audit") return <Audit />; if (kind === "profile") return <Profile />; if (kind === "notifications") return <Notifications />; return <SettingsPage />; }
...
export default function LegacyERP({ module }: { module: string }) {
  ...
  const [collapsed, setCollapsed] = useState(false); const [mobile, setMobile] = useState(false); const [profile, setProfile] = useState(false); const [createOpen, setCreateOpen] = useState(false); const [commandOpen, setCommandOpen] = useState(false); const [toast, setToast] = useState("");
  useEffect(() => { const onKey = (event: KeyboardEvent) => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "k") { event.preventDefault(); setCommandOpen(true); } if (event.key === "Escape") { setCommandOpen(false); setCreateOpen(false); setProfile(false); } }; window.addEventListener("keydown", onKey); return () => window.removeEventListener("keydown", onKey); }, []);
  useEffect(() => { if (!toast) return; const id = window.setTimeout(() => setToast(""), 3500); return () => window.clearTimeout(id); }, [toast]);
  const completeAction = (message: string) => { setCreateOpen(false); setToast(message); };
  ...
  return <div className={`erp-shell ${collapsed ? "erp-collapsed" : ""}`}><a className="erp-skip-link" href="#erp-main-content">Skip to main content</a>
    <aside className={`erp-sidebar ${mobile ? "erp-mobile-open" : ""}`}><div className="erp-brand"><Image src="/iet-logo.png" alt="IET" width={40} height={40} /><div><strong>REQGEN</strong><span>ERP 2.0</span></div><button className="erp-mobile-close" onClick={() => setMobile(false)} aria-label="Close navigation"><X size={20} /></button></div><nav>{modules.filter(m => !["profile", "notifications"].includes(m.key)).map(m => { const NavIcon = m.icon; return <Link href={`/erp-2/${m.key}`} className={pathname.endsWith(m.key) ? "active" : ""} key={m.key}><NavIcon size={19} /><span>{m.label}</span></Link>; })}</nav><div className="erp-sidebar-foot"><span>PRODUCTION</span><small>ReqGen ERP v2.0</small></div></aside>
    {mobile && <button className="erp-mobile-scrim" aria-label="Close navigation" onClick={() => setMobile(false)} />}
    <div className="erp-workspace"><header className="erp-topbar"><div className="erp-topbar-left"><button className="erp-menu-button" onClick={() => { if (window.innerWidth < 900) setMobile(true); else setCollapsed(!collapsed); }} aria-label="Toggle navigation"><Menu size={21} /></button><HeaderGreeting section={active.title.replaceAll("&", "and")} /></div><div className="erp-clock"><Clock /></div><div className="erp-topbar-actions"><button className="erp-command-trigger" onClick={() => setCommandOpen(true)} aria-label="Open quick navigation"><Search size={16} /><span>Quick search</span><kbd>Ctrl K</kbd></button><Link href="/erp-2/notifications" className="erp-icon-button erp-bell" aria-label="Notifications"><Bell size={19} /><i>5</i></Link><div className="erp-profile"><button onClick={() => setProfile(!profile)} aria-expanded={profile}><span className="erp-avatar">I</span><span><strong>Isallaah360</strong><small>Super Administrator</small></span><ChevronDown size={15} /></button>{profile && <div className="erp-profile-menu"><Link href="/erp-2/profile"><UserRound size={16} />My Profile</Link><Link href="/erp-2/settings"><Settings size={16} />Settings</Link><Link href="/login"><LogOut size={16} />Logout</Link></div>}</div></div></header>
      <main className="erp-main" id="erp-main-content"><div className="erp-watermark"><Image src="/be-logo.png" alt="" fill sizes="70vw" /></div><div className="erp-page-head"><div><span className="erp-eyebrow"><Icon size={15} />{active.label}</span><h1>{active.title}</h1><p>{active.subtitle}</p></div><div className="erp-page-actions"><button className="erp-button erp-button-secondary" onClick={() => setToast("Export prepared successfully")}><Download size={16} />Export</button>{active.action && <button className="erp-button erp-button-gold" onClick={() => setCreateOpen(true)}><Plus size={16} />{active.action}</button>}</div></div>{!["profile", "settings", "notifications", "reports"].includes(active.key) && <div className="erp-toolbar"><div className="erp-search"><Search size={17} /><input placeholder={`Search ${active.label.toLowerCase()}...`} /></div><select aria-label="Category"><option>All Categories</option></select><select aria-label="Status"><option>All Status</option></select><select aria-label="Period"><option>This Month</option></select><button className="erp-button erp-button-secondary"><SlidersHorizontal size={16} />Filter</button></div>}<ModuleContent kind={active.key} /></main>
      <footer className="erp-footer"><span>© 2026 Barderian Enterprises. All rights reserved.</span><span>ReqGen ERP 2.0 · Release 12 · Production</span></footer></div>{createOpen && <EnterpriseModal active={active} onClose={() => setCreateOpen(false)} onComplete={completeAction} />}{commandOpen && <CommandPalette onClose={() => setCommandOpen(false)} />}{toast && <div className="erp-toast" role="status"><CheckCircle2 size={19} /><div><strong>ACTION COMPLETED</strong><span>{toast}</span></div><button onClick={() => setToast("")} aria-label="Dismiss notification"><X size={16} /></button></div>}
  </div>;
}
